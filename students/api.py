...
Provides endpoints for login, token refresh, logout, and token verification.
"""
...
def api_verify_token(request):
    """
    API endpoint to verify if access token is valid.
    
    Expected POST data: {"token": "..."} OR Authorization header with Bearer token
    """
    try:
        token = None
...
def api_verify_token_get(request):
    """
    GET endpoint to verify if access token is valid (from Authorization header).
    """
    try:
        auth_header = request.META.get('HTTP_AUTHORIZATION', '')
